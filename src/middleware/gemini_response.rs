...
use crate::types::gemini_cli_schema::CliEnvelope;
use crate::types::gemini_native_schema::GeminiResponse;
use axum::{
    Json,
    http::StatusCode,
    response::{
...
) -> Result<(StatusCode, Json<GeminiResponse>), NexusError> {
...
pub fn build_stream_response(upstream_resp: reqwest::Response) -> impl IntoResponse {
    ...
    let timed_stream = transform_stream(raw_stream)
        ...
        .map(|item| match item {
            ...
            Ok(Err(e)) => Err(NexusError::StreamProtocolError(e.to_string())),
            Err(_) => {
                error!("Upstream SSE stream timed out (idle > 60s)");
                Err(NexusError::StreamProtocolError(
    ...
    I: Stream<Item = Result<eventsource_stream::Event, E>>,
{
    s.try_filter_map(|upstream_event| async move {
        ...
        let Ok(cli_resp) = serde_json::from_str::<CliEnvelope>(&upstream_event.data) else {
            warn!(
        ...
        let gemini_resp: GeminiResponse = cli_resp.into();
...
pub async fn transform_nostream(
    upstream_resp: reqwest::Response,
) -> Result<GeminiResponse, NexusError> {
    let envelope = upstream_resp.json::<CliEnvelope>().await?;
    Ok(envelope.into())
}
